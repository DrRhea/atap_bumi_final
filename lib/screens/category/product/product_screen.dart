...
class _ProductScreenState extends State<ProductScreen>
  ...
  String _sortBy = 'name'; // Filter state variables - hanya untuk harga
  RangeValues _priceRange = const RangeValues(0, 2000000);
  final double _minPriceLimit = 0;
  final double _maxPriceLimit = 2000000;
  ...
  Future<void> _loadEquipments({bool refresh = true}) async {
    // Debug print untuk memastikan filter parameter benar
    print('Filter Debug:');
    print('- minPrice: ${_priceRange.start}');
    print('- maxPrice: ${_priceRange.end}');
    print(
  ...
  Widget _buildFilterInfo() {
    ...
    return Container(
      ...
      child: Row(
        children: [
          const Icon(Icons.filter_list, size: 18, color: Color(0xFF7CB342)),
          ...
          TextButton(
            onPressed: () {
              ...
              Navigator.pushReplacement(
                ...
                MaterialPageRoute(
                  builder:
                      (context) => const ProductScreen(title: 'All Equipment'),
                ),
              );
            },
                ...
                itemBuilder: (context, index) {
                  ...
                  return GestureDetector(
                    ...
                    child: AnimatedContainer(
                      ...
                      decoration: BoxDecoration(
                        color:
                            isSelected ? const Color(0xFF7CB342) : Colors.white,
        ...
        return RefreshIndicator(
          ...
          child: GridView.builder(
            ...
            itemCount:
                ...
                (equipmentProvider.hasMoreData ? 1 : 0),
            itemBuilder: (context, index) {
                  ...
                  child: Stack(
                    children: [
                      SizedBox(
                        ...
                        child:
                            equipment.primaryPhotoUrl.isNotEmpty
                                ? Image.network(
  ...
  Widget _buildPlaceholderImage(String name) {
    ...
    } else if (name.toLowerCase().contains('bag') ||
        name.toLowerCase().contains('backpack')) {
      icon = Icons.backpack_outlined;
      bgColor = const Color(0xFF3B82F6);
    } else if (name.toLowerCase().contains('shoe') ||
        name.toLowerCase().contains('boot')) {
      icon = Icons.hiking;
      bgColor = const Color(0xFFF59E0B);
    } else if (name.toLowerCase().contains('jacket') ||
        name.toLowerCase().contains('cloth')) {
      icon = Icons.checkroom_outlined;
      bgColor = const Color(0xFF8B5CF6);
    } else if (name.toLowerCase().contains('lamp') ||
        name.toLowerCase().contains('light')) {
      icon = Icons.flashlight_on_outlined;
      bgColor = const Color(0xFFE53E3E);
    } else if (name.toLowerCase().contains('cook') ||
        name.toLowerCase().contains('stove')) {
  ...
  // Method untuk menghitung filter aktif
  int _getActiveFiltersCount() {
    int count = 0;
    if (_priceRange.start > _minPriceLimit || _priceRange.end < _maxPriceLimit)
      count++;
    return count;
  }

  // Method untuk menampilkan filter aktif
  Widget _buildActiveFilters() {
    if (_getActiveFiltersCount() == 0) {
      return const SizedBox.shrink();
    }

    List<Widget> activeFilters = []; // Price range filter
    if (_priceRange.start > _minPriceLimit ||
        _priceRange.end < _maxPriceLimit) {
      activeFilters.add(
        _buildFilterChip(
          'Price: Rp${_priceRange.start.toInt().toString().replaceAllMapped(RegExp(r'(\d{1,3})(?=(\d{3})+(?!\d))'), (Match m) => '${m[1]}.')} - Rp${_priceRange.end.toInt().toString().replaceAllMapped(RegExp(r'(\d{1,3})(?=(\d{3})+(?!\d))'), (Match m) => '${m[1]}.')}',
          () => _clearPriceFilter(),
        ),
      );
    }
        ...
        children: [
          Text(
            ...
            style: TextStyle(
              ...
              fontWeight: FontWeight.w600,
              color:
                  isSpecial ? const Color(0xFFE53E3E) : const Color(0xFF7CB342),
              fontFamily: 'Alexandria',
            ),
          ),
                            ...
                            Row(
                              children: [
                                ...
                                TextButton(
                                  onPressed: () {
                                    print(
                                      'Apply button pressed. Final price range: ${_priceRange.start} - ${_priceRange.end}',
                                    );
  ...
  // Widget filter untuk price range
    ...
    return Column(
      ...
      children: [
        ...
        Wrap(
          ...
          runSpacing: 8,
          children: [
            _buildPricePresetButton('Under 50K', 0, 50000, setModalState),
            _buildPricePresetButton('50K - 100K', 50000, 100000, setModalState),
            _buildPricePresetButton(
              '100K - 250K',
              100000,
              250000,
              setModalState,
  ...
  Widget _buildPricePresetButton(
    String label,
    double min,
    double max,
    ...
    return GestureDetector(
      onTap: () {
        print('Preset button tapped: $label (min: $min, max: $max)');
        setModalState(() {
        ...
        print('Price range updated: ${_priceRange.start} - ${_priceRange.end}');
      },
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          border: Border.all(
            color:
                isSelected ? const Color(0xFF7CB342) : const Color(0xFFE2E8F0),
            width: 1.5,
          ),
        ),
        child: Text(
